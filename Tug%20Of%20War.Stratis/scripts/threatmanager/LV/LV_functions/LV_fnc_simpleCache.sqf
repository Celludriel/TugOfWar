	...
	for[{_i=0}, {_i<(count _ids)}, {_i=_i+1}] do{
		...
		if((isNil("LVgroup"+(str (_ids select _i))+"spawned"))&&(_inRange))then{
			...
			if(_script == "militarize")then{
				call compile format["nul = %1 execVM 'scripts\threatmanager\LV\militarize.sqf';",_params];
			}else{
				call compile format["nul = %1 execVM 'scripts\threatmanager\LV\fillHouse.sqf';",_params];
			};
