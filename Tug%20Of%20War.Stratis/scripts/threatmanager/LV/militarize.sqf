	...
	for "_i" from 1 to _menAmount do{
		...
		if(!_still)then{
			if(_unitType in _menArray)then{
				nul = [_unit,_cPos,_radius,_doorHandling] execVM "scripts\threatmanager\LV\patrol-vD.sqf";
			}else{
				nul = [_unit,_pos] execVM 'scripts\threatmanager\LV\patrol-vH.sqf';
			};
		};
			...
			if(_side > 0)then{
				if(surfaceIsWater _pos)then{
					if(_vehicles select 1)then{
						...
						if(!_still)then{nul = [vehicle _driver,_pos] execVM 'scripts\threatmanager\LV\patrol-vH.sqf';};
						_validPos = true;
					};
				}else{
					if((_vehicles select 0)&&(_vehicles select 2))then{
						...
						if(_whichOne < 3)then{
							...
							if(!_still)then{nul = [_driver,_pos,[200,200]] execVM 'scripts\threatmanager\LV\patrol-vE.sqf';};
							vehicle _driver flyInHeight 10;
							_validPos = true;
						}else{
							_driver = [_pos, (_side - 1)] call LV_fullLandVehicle;
							if(!_still)then{nul = [vehicle _driver,_pos] execVM 'scripts\threatmanager\LV\patrol-vE.sqf';};
							_validPos = true;
						};
					}else{
				...
				if(!surfaceIsWater _pos)then{
					...
					if(!_still)then{nul = [_driver,_pos] execVM 'scripts\threatmanager\LV\patrol-vE.sqf';};
					_validPos = true;
				};
			};
